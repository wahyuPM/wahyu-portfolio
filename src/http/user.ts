...
export interface EducationsDoc {
  id: string;
  data: {
    school: string;
    degree: string;
...
export const fetchEducation = async (
  userId: string
): Promise<EducationsDoc | null> => {
  const educationCol = collection(db, `users/${userId}/educations`);
  const querySnapshot = await getDocs(educationCol);
  const docSnap = querySnapshot.docs[0];
  if (docSnap) {
    const docData = docSnap.data();
    if (Array.isArray(docData.data)) {
...
export const fetchCertifications = async (
  userId: string
): Promise<CertificationsDoc | null> => {
  const certificationsCol = collection(db, `users/${userId}/certifications`);
  const querySnapshot = await getDocs(certificationsCol);
  const docSnap = querySnapshot.docs[0];
  if (docSnap) {
    const docData = docSnap.data();
    if (Array.isArray(docData.data)) {
...
export async function fetchUserAndSkills(userId: string): Promise<{
  ...
  certifications: CertificationsDoc | null;
}> {
  const [user, skills, works, educations, projects, certifications] =
    await Promise.all([
      fetchUser(),
      fetchSkills(userId),
      fetchWorks(userId),
  ...
  return { user, skills, works, educations, projects, certifications };
}
